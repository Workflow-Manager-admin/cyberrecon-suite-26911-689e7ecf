...
function ReconDashboard() {
  ...
  return (
    ...
    >
      ...
      {/** FORM: Add/Edit schedule/job for recurrent/one-off scans **/}
      {/* PUBLIC_INTERFACE */}
      function ScheduleForm({ onJobAdded, errorState, editingJob }) {
        const [jobDomains, setJobDomains] = useState(editingJob?.targets?.join(", ") || "");
        const [tool, setTool] = useState(editingJob?.tool || "Amass");
        const [schedType, setSchedType] = useState(editingJob?.schedule?.type || "interval");
        const [intervalMins, setIntervalMins] = useState(editingJob?.schedule?.intervalMinutes || 60);
        ...
        const [oneTimeDate, setOneTimeDate] = useState(""); // e.g. "2024-06-30T20:00"
